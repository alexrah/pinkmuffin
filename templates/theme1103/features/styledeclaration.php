...
class GantryFeatureStyleDeclaration extends GantryFeature {
	...
	function init() {
	    ...
	    $css .= '#rt-copyright .rt-container {background:'.$gantry->get('copyright_row-container').';}';		
			...
			//Readmore style
       		$css .= 'a.moduleItemReadMore, a.k2ReadMore,div.moduleItemReadMore a,.component-content div.toggle-editor a {color:'.$gantry->get('more-default').'; background:'.$gantry->get('more-bg').';}';
       		$css .= 'a.moduleItemReadMore:hover, a.k2ReadMore:hover,div.moduleItemReadMore a:hover,.component-content div.toggle-editor a:hover {color:'.$gantry->get('more-hover').';background:'.$gantry->get('more-bg_hover').';}';
		...
		if($gantry->get('top_menu-enabled') == '1'){
       		$css .= '.menutop li.root > .item {background:'.$gantry->get('top_menu-topMenuLinkBg').';color:'.$gantry->get('top_menu-topMenuLink').';}';
